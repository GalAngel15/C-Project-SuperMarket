...
int	writeProductToBFile(FILE* fp, const Product* pProduct)
{
	if (fwrite(pProduct, sizeof(Product), 1, fp) != 1)
	{
		puts("Error writing product");
		return 0;
	}
	return 1;
}

int readProductFromBFile(FILE* fp, Product* pProduct)
{
	if (fread(pProduct, sizeof(Product), 1, fp) != 1)
	{
		printf("Error reading product\n");
		return 0;
	}
