...
int readAddressFromBFile(FILE* fp, Address* location)
{
	if (!readIntFromBFile(&location->num, fp, "Error is Reading house Number\n"))
	{
		//fclose(fp);
		return 0;
	}

	location->street = readStringFromBFile(fp, "Error in Reading Street Name\n");
	if (!location->street)
	{
		//fclose(fp);
		return 0;
	...
	location->city = readStringFromBFile(fp, "Error in Reading City Name\n");
	if (!location->city)
	{
		//fclose(fp);
		return 0;
...
int writeAddressToBFile(FILE* fp, Address* location)
{
	// Write House Number
	if (!writeIntToBFile(location->num, fp, "Error in Writing Address Number\n"))
	{
		//fclose(fp);
		return 0;
	}
	...
	// Write Street Name
	if (!writeStringToBFile(location->street, fp, "Error in writing Street Address\b"))
	{
		//fclose(fp);
		return 0;
	}
	...
	// Write City Name
	if (!writeStringToBFile(location->city, fp, "Error in Writing city name\n"))
	{
		//fclose(fp);
		return 0;
	}
